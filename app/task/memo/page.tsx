...
const createFreshCards = (): Cards => [
  ...
  { value: '6', visible: false }, { value: '6', visible: false },
];
        ...
        <div className="h-fit w-fit p-4 mt-8 border-2 border-slate-200 bg-slate-50 rounded">
          <div className='grid grid-cols-4 gap-4'>
            {cards.map((card, index) => (
              <div className={cn(card.visible && 'flipped', 'flip-card')}> 
                <Button
                  key={index}
                ...
                >
                  {card.visible ? <span style={{transform: 'scaleX(-1)'}} className={'text-lg'}>{card.value}</span> : <SpadeIcon/>}
                </Button>
              </div>
            ))}
          </div>
